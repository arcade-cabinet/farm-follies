...
import type { BushState, AnimalState, ProjectileState } from '../state/GameState';
...
export function updateBushGrowth(
  ...
  const newScale = config.initialSize + 
    (config.maxSize - config.initialSize) * newGrowthStage;
  
  // Bounce strength increases with growth
  const newBounceStrength = config.baseBounceStrength + 
 ...
 */
export function addBushToState(
  state: BushRuntimeState,
  bush: BushState
): BushRuntimeState {
 ...
 */
export function removeBushFromState(
  state: BushRuntimeState,
  bushId: string
): BushRuntimeState {
  const newBushes = new Map(state.bushes);
