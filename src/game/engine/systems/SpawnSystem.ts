...
import type { AnimalState, AnimalType, PowerUpState, TornadoState } from "../state/GameState";
...
/**
 * Reset the animal ID counter (call at game start)
 */
export function resetAnimalIdCounter(): void {
  animalIdCounter = 0;
}
...
export function selectPowerUpType(): PowerUpType {
  ...
  for (const type of types) {
    ...
    if (config && "spawnWeight" in config) {
      weights.push([type, config.spawnWeight]);
...
/**
 * Reset the power-up ID counter (call at game start)
 */
export function resetPowerUpIdCounter(): void {
  powerUpIdCounter = 0;
}

export function createPowerUpState(type: PowerUpType, x: number, y: number): PowerUpState {
  return {
