...
class AudioManager {
  ...
  async play(type: SoundType): Promise<void> {
    ...
    switch (type) {
      ...
      case "lifeup":
        ...
        break;
      case "bank":
        this.playBankSound(now);
        break;
      case "miss":
        this.playMissSound(now);
        break;
      case "topple":
        this.playToppleSound(now);
        break;
      case "ui_click":
        this.playUIClickSound(now);
        break;
      case "ui_back":
        this.playUIBackSound(now);
        break;
      case "ui_toggle":
        this.playUIToggleSound(now);
        break;
    }
  }
  ...
  private playBankSound(time: number): void {
    // Cash register cha-ching
    if (this.mainSynth) {
      this.mainSynth.triggerAttackRelease(["E5", "G5", "B5"], "16n", time, 0.5);
      this.mainSynth.triggerAttackRelease(["G5", "B5", "E6"], "8n", time + 0.12, 0.6);
    }
    if (this.fmSynth) {
      this.fmSynth.triggerAttackRelease("E6", "16n", time + 0.1, 0.3);
    }
  }

  private playMissSound(time: number): void {
    // Quick descending whiff
    if (this.mainSynth) {
      this.mainSynth.triggerAttackRelease("C5", "32n", time, 0.3);
      this.mainSynth.triggerAttackRelease("A4", "32n", time + 0.05, 0.25);
      this.mainSynth.triggerAttackRelease("F4", "16n", time + 0.1, 0.2);
    }
    if (this.noiseSynth) {
      this.noiseSynth.triggerAttackRelease("16n", time, 0.2);
    }
  }

  private playToppleSound(time: number): void {
    // Dramatic crash with rumble
    if (this.noiseSynth) {
      this.noiseSynth.triggerAttackRelease("4n", time, 0.6);
    }
    if (this.bassSynth) {
      this.bassSynth.triggerAttackRelease("C2", "4n", time, 0.7);
      this.bassSynth.triggerAttackRelease("G1", "4n", time + 0.15, 0.6);
    }
    if (this.mainSynth) {
      this.mainSynth.triggerAttackRelease(["C4", "Db4", "Gb4"], "8n", time + 0.05, 0.4);
    }
  }

  private playUIClickSound(time: number): void {
    // Short, bright tap sound
    if (this.mainSynth) {
      this.mainSynth.triggerAttackRelease("G5", "32n", time, 0.3);
    }
    if (this.noiseSynth) {
      this.noiseSynth.triggerAttackRelease("64n", time, 0.15);
    }
  }

  private playUIBackSound(time: number): void {
    // Descending two-note dismiss
    if (this.mainSynth) {
      this.mainSynth.triggerAttackRelease("E5", "32n", time, 0.25);
      this.mainSynth.triggerAttackRelease("B4", "16n", time + 0.06, 0.2);
    }
  }

  private playUIToggleSound(time: number): void {
    // Quick blip with overtone
    if (this.mainSynth) {
      this.mainSynth.triggerAttackRelease(["C5", "E5"], "32n", time, 0.25);
    }
  }
