...
function drawFunnelCloud(
  ...
  for (let layer = 0; layer < 3; layer++) {
    ...
    for (let i = 0; i <= segments; i++) {
      ...
      const widthAtSegment = topWidth - (topWidth - bottomWidth) * Math.pow(t, 0.7);
...
function drawFunnelHighlights(
  ...
  ctx.quadraticCurveTo(
    Math.cos(highlightPhase + 1) * w * 0.2,
    h * 0.5,
    w * 0.05,
    h * 0.9
  );
...
export function updateTornadoState(
  ...
  // Update position
  let newX = state.x + state.direction * config.baseSpeed * (0.5 + state.intensity * 0.5) * (deltaTime / 16);
  ...
  // Update rotation
  const newRotation = state.rotation + config.rotationSpeed * (deltaTime / 16) * (0.5 + state.intensity * 0.5);
