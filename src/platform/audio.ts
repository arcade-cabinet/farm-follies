...
const SFX_FILES: Record<SoundType, string> = {
  ...
  miss: `${AUDIO_BASE}/sfx/miss.ogg`,
  topple: `${AUDIO_BASE}/sfx/topple.ogg`,
  ui_click: `${AUDIO_BASE}/ui/click.ogg`,
  ui_back: `${AUDIO_BASE}/ui/back.ogg`,
  ui_toggle: `${AUDIO_BASE}/ui/toggle.ogg`,
};
...
const BACKGROUND_MUSIC_FILE = `${AUDIO_BASE}/music/background.wav`;
...
class PlatformAudioManager {
  ...
  private backgroundMusicAvailable = false;
  ...
  async init(): Promise<void> {
    ...
    // Attempt to load background music (independent of main files check)
  ...
  /**
   * Preload the background music WAV file (used for menu/ambient)
   */
  private async preloadBackgroundMusic(): Promise<void> {
    try {
      const response = await fetch(BACKGROUND_MUSIC_FILE, { method: "HEAD" });
      if (!response.ok) {
        console.log("[Audio] Background music file not available");
        return;
      }
      ...
      this.backgroundMusicAvailable = true;
      console.log("[Audio] Background music loaded");
    } catch (error) {
      console.warn("[Audio] Failed to preload background music:", error);
    }
  }

  /**
   * Start background/menu music loop (the background.wav track)
   * This is separate from the intensity-based game music.
   */
  startBackgroundMusic(volume = 0.3): void {
    if (this.muted || this.backgroundMusicPlaying || !this.backgroundTrack) return;
  ...
  /**
   * Stop background/menu music loop
   */
  stopBackgroundMusic(): void {
    if (!this.backgroundMusicPlaying || !this.backgroundTrack) return;
  ...
  /**
   * Check if background music file is available
   */
  isBackgroundMusicAvailable(): boolean {
    return this.backgroundMusicAvailable;
  }
  ...
  dispose(): void {
    ...
    this.backgroundMusicAvailable = false;
